...
class BookAnAppointmentViews(SuccessMessageMixin,View):
    def post(self,request, *args, **kwargs):
        if request.method == "POST":
            ...
            hospitalservice = ServiceAndCharges.objects.get(id=serviceid)
            date = request.POST.get('time')
            time = request.POST.get('date') 
            print(doctorid,hospitalstaffdoctor,serviceid,hospitalservice,date,time)
            booking = Booking(patient = request.user,hospitalstaffdoctor=hospitalstaffdoctor,service=hospitalservice,applied_date=date,applied_time=time,is_applied=True,is_active=True,amount=hospitalservice.service_charge)
            print("booking saved")
            booking.save()
...
class labDetailsViews(DetailView):
    def get(self, request, *args, **kwargs):
        ...
        service = ServiceAndCharges.objects.filter(user=lab.admin)        
        param = {'lab':lab,'service':service}  
